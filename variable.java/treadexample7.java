...
public class treadexample7 {
        ...
        public void run() {
            try {
                for (int i = 1; i <= total; i++) {
                    Thread.sleep(150 + rnd.nextInt(300)); // simulate preparation time
    ...
    static class Customer implements Runnable {
        ...
        public void run() {
            try {
                while (true) {
                    Pizza p = box.take();
                    if (p == null) break; // no more pizzas will come
                    System.out.println(Thread.currentThread().getName() + " is eating " + p);
                    Thread.sleep(200 + rnd.nextInt(400)); // simulate eating time
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            } finally {
    ...
    public static void main(String[] args) throws InterruptedException {
        PizzaBox box = new PizzaBox(3); // box capacity
        int totalPizzas = 12;
